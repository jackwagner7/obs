...
async function getArtistTracks(){
  let response1 = await spotifyApi.getArtistAlbums("46gyXjRIvN1NL1eCB8GBxo");
  console.log(response1);
  var tracks = [];
  for(var i = 0; i<response1.items.length; i++){
    console.log(response1.items[i])
    if(response1.items[i].album_group !== "appears_on"){
      var albumID = response1.items[i].id;
      let albumTracks = await spotifyApi.getAlbumTracks(albumID);
      for(var j = 0; j<albumTracks.items.length; j++){
        tracks.push(albumTracks.items[j])
      }
    }
  }
  return tracks;
